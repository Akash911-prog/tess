...
class Indicator(tk.Tk):
    def __init__(self, state_manager: ContextManager, geometry: str | None = '20x20+1850+20') -> None:
        super().__init__()

        self.title("Indicator")
        self.geometry(geometry)
        self.state = state_manager
        self.status = self.state.session.state
...
if __name__ == "__main__":
    indicator = Indicator(ContextManager())
    time.sleep(3)
    indicator.render()